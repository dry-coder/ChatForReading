$(document).ready(function () {
    ...
    function AppViewModel() {
        ...
        self.filter_room = ko.observable("");
        self.filteredChatRooms = ko.computed(function () {
            if (!self.filter_room()) {
                return self.chatRooms();
            } else {
                return ko.utils.arrayFilter(self.chatRooms(), function (user) {
                    var fullName = user.name().toLowerCase();
                    return fullName.includes(self.filter_room().toLowerCase());
                });
            }
        }); 

        //txt文本搜索的内容
        self.filter_content = ko.observable("");
